...
const Logoipsum = ({ icon }: { icon: React.ReactNode }) => (
  <div className="flex items-center gap-2">
    {icon}
    <span className="font-bold text-[22px] text-[#21282C] dark:text-gray-200 tracking-tight">Logoipsum</span>
  </div>
);
...
export default function PartnersSection() {
    const logos = [
        <div key="189" className="flex items-center gap-1.5">
            ...
            <span className="text-[24px] font-black italic text-[#1A202C] dark:text-white tracking-tight">TAXI</span>
        </div>,
        <Logoipsum key="c" icon={<Command className="text-[#21282C] dark:text-gray-200" size={32} strokeWidth={2.5} />} />,
        <div key="apar" className="text-[26px] font-black text-[#D32F2F] flex items-center tracking-[0.1em] uppercase">
            APAR
        </div>,
        <Logoipsum key="b" icon={<Box className="text-[#21282C] dark:text-gray-200" size={32} strokeWidth={2.5} />} />,
        <Logoipsum key="h" icon={<Hexagon className="text-[#21282C] dark:text-gray-200" size={32} strokeWidth={2.5} />} />,
        <Logoipsum key="cd" icon={<CircleDashed className="text-[#21282C] dark:text-gray-200" size={32} strokeWidth={2.5} />} />,
        <Logoipsum key="t" icon={<Triangle className="text-[#21282C] dark:text-gray-200" size={32} strokeWidth={2.5} />} />,
    ...
    return (
        <section className="bg-[#F4F8FA] dark:bg-[#121212] py-16 overflow-hidden transition-colors duration-300">
            <style>{`
                @keyframes slide {
            ...
            <div className="container mx-auto px-4 text-center mb-12" data-aos="fade-up">
                <p className="text-[#21282C] dark:text-gray-300 text-[15px] font-medium tracking-wide">More than 320 trusted partners & clients</p>
            </div>
            ...
            <div className="w-full relative flex items-center group">
                ...
                <div className="flex w-max animate-slide hover:[animation-play-state:paused] gap-16 md:gap-24 px-8 items-center cursor-pointer">
                    {allLogos.map((logo, index) => (
                        <div 
                            key={index} 
                            className="flex-shrink-0 flex items-center justify-center grayscale opacity-60 hover:grayscale-0 hover:opacity-100 dark:opacity-40 dark:hover:opacity-100 transition-all duration-300"
                        >
                            {logo}
                        </div>
                    ))}
