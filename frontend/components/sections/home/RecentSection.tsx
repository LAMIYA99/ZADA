            ...
            <div className="mx-auto">
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-16 items-start">
                    <div className="lg:col-span-4 flex items-center gap-4">
                        ...
                        <span className="px-5 py-2 bg-[#21282C] dark:bg-white text-white dark:text-[#1D1D24] rounded-full text-sm font-medium">Resources</span>
                    </div>
                    <div className="lg:col-span-6 lg:col-start-5" data-aos="fade-up">
                        <h2 className="text-4xl md:text-[40px] font-semibold mb-4 text-[#1A202C] dark:text-white">Recent news</h2>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {recentArticles.map((article, index) => (
                        <Link href={`/detail?id=${article.id}`} key={article.id} className="group cursor-pointer block" data-aos="fade-up" data-aos-delay={index * 100}>
                            <div className="relative overflow-hidden mb-4 bg-gray-100 dark:bg-gray-800 h-[520px] w-full shrink-0">
                                <img
                                    src={article.image}
                                    alt={article.title}
                            ...
                            <div>
                                ...
                                <h3 className="text-[22px] font-bold text-[#1A202C] dark:text-white leading-snug group-hover:text-[#8bc22b] dark:group-hover:text-[#C1FE72] transition-colors">{article.title}</h3>
                            </div>
                        </Link>
                    ))}
                </div>
