...
export default function FeaturedSection() {
  ...
  return (
    <section className="w-full bg-[#f4f5f6] dark:bg-[#121212] text-[#1D1D24] dark:text-white py-24 lg:py-32 overflow-hidden transition-colors duration-300">
        ...
        <div className="flex flex-col md:flex-row relative mb-20 gap-10 lg:gap-0">
          ...
          <div className="md:w-full flex justify-center" data-aos="fade-up">
            <div className="max-w-2xl text-left md:pl-32 lg:pl-0 pt-2 md:pt-0">
              ...
              <p className="text-gray-500 dark:text-gray-400 text-[16px] lg:text-[16px] leading-relaxed mx-auto max-w-[600px]">
      ...
      <div className="w-full relative overflow-visible pl-6 md:pl-12 xl:pl-[calc((100vw-1710px)/2+48px)]" ref={emblaRef}>
        <div className="flex touch-pan-y cursor-grab active:cursor-grabbing pb-4 select-none">
          {projects.map((project, idx) => (
            <Link href={`/projects/${project.slug}`} key={idx} className="flex-[0_0_auto] w-[397px] min-w-0 mr-8 last:mr-0 group cursor-pointer block">
              <div className="relative w-full h-[286px] overflow-hidden mb-6 bg-gray-200 dark:bg-gray-800">
                <img 
                  src={project.image} 
                  alt={project.title} 
              ...
              <div className="flex flex-col items-start">
                <h3 className="text-[36px] font-bold mb-4 text-[#1D1D24] dark:text-white tracking-tight">{project.title}</h3>
                <div className="flex flex-wrap gap-2">
                  {project.categories.map((cat, i) => (
                    <span key={i} className="px-4 py-1.5 border border-gray-200 dark:border-gray-700 rounded-[30px] text-[16px] font-medium text-gray-800 dark:text-gray-300 bg-transparent">
                      {cat}
                    </span>
                  ))}
                </div>
